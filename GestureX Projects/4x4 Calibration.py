...
# Eye landmark indices for simple EAR calculation (MediaPipe face mesh)
# Each list: [horizontal_left, upper, lower, horizontal_right]
LEFT_EYE_LANDMARKS = [362, 385, 387, 263]
RIGHT_EYE_LANDMARKS = [33, 160, 158, 133]
...
def calculate_ear(landmarks, indices):
    # indices: [left, top, bottom, right]
    # We use the distance between top/bottom divided by left/right
...
def force_window_front(win_name):
    user32 = ctypes.windll.user32
...
def mode_calibration(num_points=16):
    ...
    cv2.imshow(win, make_fullscreen_canvas(255)) 
    cv2.waitKey(1)
    bring_window_to_front(win)
...
def mode_cursor_control():
    # Updated refined indices for the 478-point model
    LEFT_EYE_REFINED = [362, 386, 374, 263]  # Left-edge, Top, Bottom, Right-edge
    RIGHT_EYE_REFINED = [33, 159, 145, 133]  # Left-edge, Top, Bottom, Right-edge
    ...
    # FIX: Toggled to True to correct the X-axis inversion
    invert_x = True 
    ...
    # Separate states for both eyes
    is_left_winked = False
    is_right_winked = False

    print("Cursor Control Active. Press 'CTRL+Q' anywhere to exit.")

    while True:
        # UNIVERSAL EXIT CHECK
        # This works even if the terminal or the preview window is NOT in focus
        ...
        if results.multi_face_landmarks:
            ...
            # 1. VISUAL FEEDBACK (Red dots on eyelid tracking points)
            for idx in (LEFT_EYE_REFINED + RIGHT_EYE_REFINED):
                pt = landmarks[idx]
            ...
            # 3. WINK DETECTION (Dual Eye Support)
            l_ear = calculate_ear(landmarks, LEFT_EYE_REFINED)
            r_ear = calculate_ear(landmarks, RIGHT_EYE_REFINED)
            
            # Differential EAR helps prevent accidental clicks from normal blinking
            l_diff = r_ear - l_ear
            r_diff = l_ear - r_ear
            current_ms = time.time() * 1000

            # LEFT WINK (Physical Right Eye) -> LEFT CLICK
            if l_ear < 0.18 and l_diff > 0.07:
            ...
            # RIGHT WINK (Physical Left Eye) -> RIGHT CLICK
            if r_ear < 0.18 and r_diff > 0.07:
            ...
            cv2.putText(frame, f"L: {l_ear:.2f} R: {r_ear:.2f}", (10, 30), 
        ...
        # Keep waitKey to allow OpenCV to render the window
        cv2.waitKey(1)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if mode == "1":
            ...
            print("Invalid mode argument. Use 1, 2 or 3.")
    else:
